    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-6">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white shadow rounded-lg p-6 text-center">
                ...
                <p class="text-xs text-gray-400 mt-1">{{ ucfirst($user->role) }}</p>
            </div>

            {{-- Candidate Attachments (only for candidates) --}}
            @if($user->isCandidate())
                                ...
                                for (const file of files){
                                    ...
                                    try{
                                        ...
                                        const res=await axios.post('{{ route('account.upload') }}',formData,{
                                            ...
                                            onUploadProgress:(e)=>{ if(e.total){ item.progress=Math.round((e.loaded*100)/e.total); } }
                                        });
                                        if(res.data.success){
            ...
            @if($user->isEmployer())
                @php $employer = $user->employers()->first(); @endphp
                <div class="bg-white shadow rounded-lg p-6">
                    <h3 class="text-lg font-semibold mb-3">Company Details</h3>

                    @if($employer)
                        <p><strong>Company:</strong> {{ $employer->name }}</p>
                        ...
                        <a href="{{ route('employer.show', $employer) }}" class="text-blue-600 underline text-sm">View Company Profile</a>
                    @else
                        <p class="text-gray-500">You are not linked to any company yet.</p>
                        <a href="{{ route('employer.create') }}" class="text-blue-600 underline text-sm">Create Company</a>
                    @endif
                </div>
            @endif
        </div>
