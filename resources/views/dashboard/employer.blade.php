            ...
            <div class="column" style="width: 49%;">
                <x-card class="!bg-gradient-to-br from-[#3b76c4] to-[#0cc0df] p-6 flex flex-col !h-[620px]">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route('jobs.index') }}" class="bg-white text-blue-600 px-4 py-2 rounded-lg font-semibold hover:bg-blue-50 transition text-sm flex items-center gap-2">
                            View All
                    ...
                    @php
                        $campaigns = \App\Models\Job::latest()->take(6)->get();
                    @endphp

                    @if($campaigns->count() > 0)
                        <div class="grid grid-cols-3 grid-rows-2 gap-4 flex-1">
                            @foreach($campaigns as $campaign)
                                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition flex flex-col">
                                    <!-- Logo -->
                                    ...
                                    <p class="text-xs mb-1 text-center opacity-90">
                                        <strong>{{ $campaign->jobApplications()->count() }}</strong> Candidates
                                    </p>

                                    <!-- Managed By -->
                                    <p class="text-xs mb-3 text-center opacity-75">
                                        Managed by: {{ $campaign->employer->user->name ?? 'Admin' }}
                                    </p>

                                    <!-- View Button -->
                                    <a href="{{ route('jobs.show', $campaign) }}" class="mt-auto bg-white text-blue-600 px-4 py-2 rounded-lg text-xs font-semibold hover:bg-blue-50 transition text-center">
                                        View Campaign
                                    </a>
                                </div>
                            @endforeach
                    ...
                    @if(auth()->user()->employer)
                        <div class="flex gap-4 justify-center">
                            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center hover:bg-white/20 transition flex-1">
                                <div class="text-4xl font-bold mb-1">{{ \App\Models\Job::count() }}</div>
                                <div class="text-xs opacity-90">Campaigns</div>
                            </div>
                            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center hover:bg-white/20 transition flex-1">
                                <div class="text-4xl font-bold mb-1">
                                    {{ \App\Models\User::where('role', 'candidate')->count() }}
                                </div>
