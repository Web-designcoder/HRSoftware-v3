<x-card title="Required Candidate Documents">
    <div x-data="jobRequiredDocs({
        fetchUrl: '{{ route('admin.jobs.reqdocs.index', $job) }}',
        uploadUrl: '{{ route('admin.jobs.reqdocs.store', $job) }}',
        deleteBase: '{{ route('admin.jobs.reqdocs.destroy', [$job, 0]) }}',
        reorderUrl: '{{ route('admin.jobs.reqdocs.reorder', $job) }}',
        csrf: '{{ csrf_token() }}'
    })">
        <template x-if="docs.length===0">
            <p class="text-sm text-gray-500">No required documents yet.</p>
        </template>
            ...
            <ul class="space-y-1">
                <template x-for="doc in docs" :key="doc.id">
                    <li class="flex justify-between items-center border p-2 rounded-md">
                        <span x-text="doc.name" class="text-sm"></span>
